...
tsuro.controller('gameCtrl', function ($scope, $firebaseAuth, firebaseUrl, $stateParams, $firebaseObject, $firebaseArray, $state, gameFactory, $window) {
  ...
  ****************/
  ...
  spaceRef.on('child_added', function (snapshot) {
    ...
    if ($scope.me) {
      if ($scope.me.x === x && $scope.me.y === y) {
        console.log("it's me.", $scope.me," let's move");
      ...
      if (!$scope.dragon) {
        ...
        console.log("is it me?", isCurrentPlayer());
  ...
  // Player has played
    playersRef.on("child_changed", function(snapshot){
      ...
      if (player.dead){
        ...
        //do for local player with cards
        if( player.uid === $scope.me.uid && $scope.me.tiles.length>0){
          console.log("I am dead, I must relinquish my tiles");
      ...
      if ($scope.game.deck.length() === 0){
        ...
        /**
        ***
        **8	The current dragon implentation won't work
        ***
        ***/
        //////		TODO: convert the logic to account for events triggering this in multiple places
    ...
    function winLoseDraw(){
      ///WIN CONDITIONS
      console.log("Players left in game:", $scope.game.getCanPlay().length);
  ...
  function isCurrentPlayer () {
    return $scope.game.currentPlayer.uid === $scope.me.uid
  }
  $scope.starttop = [
    [0, 0, 0],
    [0, 0, 1],
